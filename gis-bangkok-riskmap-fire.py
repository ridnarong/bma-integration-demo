...
r = requests.get('https://bmagis.bangkok.go.th/arcgis/rest/services/RISK_Fire/FeatureServer/0/query?where=1%3D1&objectIds=&time=&geometry=&geometryType=esriGeometryEnvelope&inSR=&spatialRel=esriSpatialRelIntersects&distance=&units=esriSRUnit_Foot&relationParam=&outFields=*&returnGeometry=true&maxAllowableOffset=&geometryPrecision=&outSR=&having=&gdbVersion=&historicMoment=&returnDistinctValues=false&returnIdsOnly=false&returnCountOnly=false&returnExtentOnly=false&orderByFields=&groupByFieldsForStatistics=&outStatistics=&returnZ=false&returnM=false&multipatchOption=xyFootprint&resultOffset=&resultRecordCount=&returnTrueCurves=false&returnExceededLimitFeatures=false&quantizationParameters=&returnCentroid=false&sqlFormat=none&resultType=&featureEncoding=esriDefault&f=pjson')
for feature in r.json()['features']:
  loc = geo.reverse_geo(feature['geometry']['x'], feature['geometry']['y'])
  lines.append(json.dumps({
    "index" : { "_index" : "search-gis-bma", "_id" : 'riskMapFire:' + feature['attributes']['GLOBALID'].replace('{', '').replace('}', '') } 
  }))
  lines.append(json.dumps({
    'id': 'riskMapFire:' + feature['attributes']['GLOBALID'].replace('{', '').replace('}', ''),
    'source': 'BMA Risk Map : Fire',
    'title': feature['attributes']['NAMERISKPLACE1'],
    'body': "%s %s %s ระดับความเสี่ยง %f จำนวนประชากร %d จำนวนอาคาร %s" % (feature['attributes']['NAMERISKPLACE1'],
                         feature['attributes']['SOINAME1'],
                         feature['attributes']['ROADNAME1'],
                         feature['attributes']['RISKLEVEL'],
                         feature['attributes']['POPULATION'],
                         feature['attributes']['SUMBUILDING1'],),
